...
metaStyle <- read_excel("analysis/Analysis.xlsx", 
                         sheet = "Metadata(modified)")
metaStyle.df <- as.data.frame(cbind(metaStyle[2],metaStyle$`Style,Primary`))


colnames(metaStyle.df) <- c("sampletext", "samplestyle")
metacols.style.df <- merge(metacols.df, metaStyle.df, 
                            by = "sampletext",
                            all.x = TRUE)
dim(metacols.style.df)
#1630 * 3
head(metacols.style.df)


#Label for genders
style.v <- metacols.style.df[, 3]
unique(style.v)
#Final data frame
style.df <- cbind(style.v, metacols.m, final.df)
dim(style.df)
#1650 * 56555
...
freq.means.v <- colMeans(style.df[,4:ncol(style.df)])
#Find columns that has mean greater or equal to 0.005
keepers.v <- which(freq.means.v >=.001)
length(keepers.v)
keepers.v
names(keepers.v)
smaller.df <- style.df[, names(keepers.v)] #does not include gender, metalcols
dim(smaller.df)
smaller.df <- style.df[, c(names(style.df)[1:3],
                            names(keepers.v))]
dim(smaller.df)
...
anon.v <- which(is.na(smaller.df$style.v))
...
class.f <- smaller.df[-anon.v,"style.v"]
...
write.csv(pred.svm.df, "analysis/supervised_classification_style/0.001-118words-style-model-labels.csv")
table <- table(pred.svm, class.f)
#Write the summary into csv
write.csv(table(pred.svm, class.f), 
          "analysis/supervised_classification_style/0.001-118words-style-svm-summary.csv")
...
write.csv(as.data.frame(final.result), 
          "analysis/supervised_classification_style/0.001-118words-style-predict-testdata.csv")


#Increase the number of features by decrease the frequency mean
keepers.v <- which(freq.means.v >=.00005)
length(keepers.v)
names(keepers.v)
smaller.df <- style.df[, c(names(style.df)[1:3],
                            names(keepers.v))]
...
pred.svm <- predict(model.svm, train)
pred.svm.df <- as.data.frame(pred.svm)
write.csv(pred.svm.df, 
          "analysis/supervised_classification_style/0.00005-2252words-style-model-labels.csv")
table(pred.svm, class.f)
write.csv(table(pred.svm, class.f), 
          "analysis/supervised_classification_style/0.0001-1124words-style-svm-summary.csv")
...
write.csv(final.result.df, 
          "analysis/supervised_classification_style/0.0001-1124words-style-predict-testdata.csv")
...
new.smaller.df <- smaller.df[,
                             c("style.v","sampletext","samplechunk", keepers.v)]
anon.v <- which(is.na(smaller.df$style.v))
train <- new.smaller.df[-anon.v,4:ncol(new.smaller.df)]
class.f <- new.smaller.df[-anon.v,"style.v"]
model.svm <- svm(train, class.f)
pred.svm <- predict(model.svm, train)
write.csv(table(pred.svm, class.f), 
